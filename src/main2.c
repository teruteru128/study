...
int hiho(int argc, char **argv, const char **envp)
{
    ...
    */
    ...
    for (size_t y = 0; y < ihdr.height; y++)
    {
        ...
        {
            data[y][x + 0]
                = (png_byte)(((double)colors[0] * (ihdr.height - y) + colors[3] * y)
                             / ihdr.height);
            data[y][x + 1]
                = (png_byte)(((double)colors[1] * (ihdr.height - y) + colors[4] * y)
                             / ihdr.height);
            data[y][x + 2]
                = (png_byte)(((double)colors[2] * (ihdr.height - y) + colors[5] * y)
                             / ihdr.height);
            // data[y][x + 3] = (png_byte)(255. * (ihdr.width - (x / 4)) / ihdr.width);
        }
    }
