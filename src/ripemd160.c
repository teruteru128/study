...
*/
static unsigned int f(int j, unsigned int x, unsigned int y, unsigned int z)
{
    switch (j >> 4)
    {
    case 0:
...
static void ripemd160_init(ripemd160_ctx *ctx, unsigned char *p, unsigned char len)
{
...
static unsigned int rol(unsigned int x, int si)
{
    return (x << si) | (x >> (32 - si));
}
