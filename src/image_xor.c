...
int main(int argc, char **argv) {
    ...
    png_bytep *rows1 = NULL, *rows2 = NULL, *rows_xor = NULL;
    ...
    rows_xor = (png_bytep*)malloc(sizeof(png_bytep) * h1);
    int channels = (color1 == PNG_COLOR_TYPE_RGBA) ? 4 : 3;
    size_t row_bytes = w1 * channels;
    for (int y = 0; y < h1; y++) {
        rows_xor[y] = (png_byte*)malloc(row_bytes);
    }

    // ピクセルごとにXOR（排他的論理和）を計算
    for (int y = 0; y < h1; y++) {
        png_bytep row1 = rows1[y];
        png_bytep row2 = rows2[y];
        png_bytep rowx = rows_xor[y];

        for (int x = 0; x < w1 * channels; x += channels) {
            // R, G, B 各チャンネルのビットXOR
            rowx[x]     = row1[x]     ^ row2[x];     // Red
            rowx[x + 1] = row1[x + 1] ^ row2[x + 1]; // Green
            rowx[x + 2] = row1[x + 2] ^ row2[x + 2]; // Blue
            
            // アルファチャンネルの処理
            if (channels == 4) {
                rowx[x + 3] = 255; // 透過させず不透明の黒背景にする
            }
        }
    }

    if (!write_png(argv[3], w1, h1, rows_xor, color1, depth1)) {
        fprintf(stderr, "Error writing output file.\n");
    } else {
        printf("Success: XOR image saved to %s\n", argv[3]);
    }

    free_rows(rows1, h1);
    free_rows(rows2, h2);
    free_rows(rows_xor, h1);
