                               ...
                               unsigned char *encpub, size_t encpublen)
{
    ...
    {
        ret[(i << 1) + 12] = a[(signpub[i + 1] >> 4) & 0x0f];
        ret[(i << 1) + 13] = a[(signpub[i + 1] >> 0) & 0x0f];
    }
    for (size_t i = 0; i < 64; i++)
    {
        ret[(i << 1) + 140] = a[(encpub[i + 1] >> 4) & 0x0f];
        ret[(i << 1) + 141] = a[(encpub[i + 1] >> 0) & 0x0f];
    }
    // 140 + 128
    // FD03E8
    ret[268] = 'f';
...
int main(int argc, char const *argv[])
{
    ...
    for (i = 0; i < 130; i += 65)
    {
        for (j = 0; j < 4362076160UL; j += 65)
        {
            ...
            EVP_DigestUpdate(mdctx, publickeys + i, 65);
            ...
            if (work[0] == 0)
            {
                ...
                transmitdatahex = buildTransmitdata(publickeys + i, 65,
                                                    publickeys + j, 65);
                if (isFirst == 1)
                {
                    isFirst = 0;
                ...
                {
                    fputs(";\n", stdout);
                    fputs("INSERT INTO pubkeys(address, addressversion, "
                          "transmitdata, time, "
                          "usedpersonally) values\n",
                          stdout);
                    isFirst = 1;
